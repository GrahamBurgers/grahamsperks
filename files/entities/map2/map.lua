...
if ( timercomp ~= nil ) and ( entity_id ~= player_id ) and ( pw == 0 ) and ( tostring(SessionNumbersGetValue("NEW_GAME_PLUS_COUNT")) == "0" ) then
	...
	if ( is_moving == false ) then
		...
		if ( timer > 80 ) then
			...
			fx = fx - 420 * 0.5
			fy = fy - 288 * 0.5
			GameCreateSpriteForXFrames( "mods/grahamsperks/files/entities/map2/" .. map_sprite .. ".png", mi_x, mi_y, true, 0, 0, 1, true )
		end
	else
		timer = 0
	end
