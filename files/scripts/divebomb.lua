...
for i, enemy in ipairs(enemies) do
    ...
    if y < y2 and math.abs(x - x2) < DISTANCE / 12 and RaytracePlatforms( x - 4, y, x2 + 4, y2) ~= true and EntityGetComponent(enemy, "PhysicsBodyComponent") == nil and whoshot ~= enemy then
        dofile_once("data/scripts/lib/utilities.lua")

        edit_component( me, "VelocityComponent", function(comp,vars)
            local vel_x,vel_y = ComponentGetValueVector2( comp, "mVelocity")
        
            vel_y = math.max(vel_y, 0) + math.abs(vel_x) * 1.2
